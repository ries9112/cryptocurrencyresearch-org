...
cryptodata_nested <- mutate(cryptodata_nested,
                            ...
                            bridge_test_predictions = ifelse(split < 5,
                                                         map2(train_data, bridge_test_predictions, last_train_price),
                                                         NA),
                            # randomGLM:
                            randomGLM_test_predictions = ifelse(split < 5,
                                                         map2(train_data, randomGLM_test_predictions, last_train_price),
                                                         NA),
                            # PCR:
...
cryptodata_nested_holdout <- mutate(filter(cryptodata_nested, split == 5),
                                    ...
                                    # Bayesian Ridge Regression:
                                    bridge_holdout_predictions = map2(train_data, bridge_holdout_predictions, last_train_price),
                                    # randomGLM:
                                    randomGLM_holdout_predictions = map2(train_data, randomGLM_holdout_predictions, last_train_price),
                                    # PCR:
...
cryptodata_nested <- left_join(cryptodata_nested, 
                               dplyr::select(cryptodata_nested_holdout, symbol, 
                                      xgb_holdout_predictions, nnet_holdout_predictions, bridge_holdout_predictions,
                                      randomGLM_holdout_predictions, pcr_holdout_predictions),
                               by='symbol')
# Remove unwanted columns
cryptodata_nested <- dplyr::select(cryptodata_nested, -xgb_holdout_predictions.x, 
                            -nnet_holdout_predictions.x, -bridge_holdout_predictions.x,
                            -randomGLM_holdout_predictions.x, pcr_holdout_predictions.x, -split.y)
# Rename the columns kept
cryptodata_nested <- rename(cryptodata_nested, 
                            xgb_holdout_predictions = 'xgb_holdout_predictions.y',
                            nnet_holdout_predictions = 'nnet_holdout_predictions.y',
                            bridge_holdout_predictions = 'bridge_holdout_predictions.y',
                            randomGLM_holdout_predictions = 'randomGLM_holdout_predictions.y',
...
cryptodata_nested <- mutate(cryptodata_nested,
                            ...
                            bridge_test_predictions = ifelse(split < 5,
                                                         map(bridge_test_predictions, standardize_perc_change),
                                                         NA),
                            # holdout - all splits
                            bridge_holdout_predictions = map(bridge_holdout_predictions, standardize_perc_change),
                            # randomGLM:
                            randomGLM_test_predictions = ifelse(split < 5,
                                                         map(randomGLM_test_predictions, standardize_perc_change),
                                                         NA),
                            # holdout - all splits
                            randomGLM_holdout_predictions = map(randomGLM_holdout_predictions, standardize_perc_change),
                            # PCR:
                            pcr_test_predictions = ifelse(split < 5,
...
cryptodata_nested <- mutate(cryptodata_nested,
                            ...
                            # Bayesian Ridge Regression - RMSE - Test
                            bridge_rmse_test = unlist(ifelse(split < 5,
                                                         map2(bridge_test_predictions, actuals_test, evaluate_preds_rmse),
                                                         NA)),
                            # And holdout:
                            bridge_rmse_holdout = unlist(map2(bridge_holdout_predictions, actuals_holdout, evaluate_preds_rmse)),
                            # Bayesian Ridge Regression - R^2 - Test
                            bridge_rsq_test = unlist(ifelse(split < 5,
                                                         map2(bridge_test_predictions, actuals_test, evaluate_preds_rsq),
                                                         NA)),
                            # And holdout:
                            bridge_rsq_holdout = unlist(map2(bridge_holdout_predictions, actuals_holdout, evaluate_preds_rsq)),
                            # randomGLM - RMSE - Test
                            randomGLM_rmse_test = unlist(ifelse(split < 5,
                                                         map2(randomGLM_test_predictions, actuals_test, evaluate_preds_rmse),
                                                         NA)),
                            # And holdout:
                            randomGLM_rmse_holdout = unlist(map2(randomGLM_holdout_predictions, actuals_holdout, evaluate_preds_rmse)),
                            # randomGLM - R^2 - Test
                            randomGLM_rsq_test = unlist(ifelse(split < 5,
                                                         map2(randomGLM_test_predictions, actuals_test, evaluate_preds_rsq),
                                                         NA)),
                            # And holdout:
                            randomGLM_rsq_holdout = unlist(map2(randomGLM_holdout_predictions, actuals_holdout, evaluate_preds_rsq)),
                            # PCR - RMSE - Test
...
<!-- rmse_scores <- dplyr::select(cryptodata_nested, lm_rmse_test, xgb_rmse_test, bridge_rmse_test, nnet_rmse_test) -->
...
<!-- rsq_scores <- dplyr::select(cryptodata_nested, lm_rsq, xgb_rsq, bridge_rsq, nnet_rsq) -->
...
rmse_test <- mutate(cryptodata_metrics,
                      ...
                      bridge = mean(bridge_rmse_test, na.rm = T),
                      randomGLM = mean(randomGLM_rmse_test, na.rm = T),
...
rmse_holdout <- mutate(cryptodata_metrics,
                      ...
                      bridge = mean(bridge_rmse_holdout, na.rm = T),
                      randomGLM = mean(randomGLM_rmse_holdout, na.rm = T),
...
rsq_test <- mutate(cryptodata_metrics,
                      ...
                      bridge = mean(bridge_rsq_test, na.rm = T),
                      randomGLM = mean(randomGLM_rsq_test, na.rm = T),
...
rsq_holdout <- mutate(cryptodata_metrics,
                      ...
                      bridge = mean(bridge_rsq_holdout, na.rm = T),
                      randomGLM = mean(randomGLM_rsq_holdout, na.rm = T),
...
```{r include=F, eval=F}
last_train_price <- function(holdout_data, holdout_predictions){
  # NEED TO ADD HERE
}
```
...
```{r include=F, eval=F}
# example:
unnest(cryptodata_nested, c(pcr_holdout_predictions, nnet_holdout_predictions)) %>% select(pcr_holdout_predictions, nnet_holdout_predictions)
```
