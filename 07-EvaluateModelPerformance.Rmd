...
cryptodata_nested <- mutate(cryptodata_nested,
                            ...
                            brnn_test_predictions = ifelse(split < 5,
                                                         map2(train_data, brnn_test_predictions, last_train_price),
                                                         NA),
                            # randomGLM:
...
cryptodata_nested_holdout <- mutate(filter(cryptodata_nested, split == 5),
                                    ...
                                    # Bayesian Regularized Neural Network:
                                    brnn_holdout_predictions = map2(train_data, brnn_holdout_predictions, last_train_price),
                                    # randomGLM:
...
cryptodata_nested <- left_join(cryptodata_nested, 
                               dplyr::select(cryptodata_nested_holdout, symbol, 
                                      xgb_holdout_predictions, nnet_holdout_predictions, brnn_holdout_predictions,
...
cryptodata_nested <- dplyr::select(cryptodata_nested, -xgb_holdout_predictions.x, 
                            -nnet_holdout_predictions.x, -brnn_holdout_predictions.x,
...
cryptodata_nested <- rename(cryptodata_nested, 
                            ...
                            brnn_holdout_predictions = 'brnn_holdout_predictions.y',
...
cryptodata_nested <- mutate(cryptodata_nested,
                            ...
                            brnn_test_predictions = ifelse(split < 5,
                                                         map(brnn_test_predictions, standardize_perc_change),
                                                         NA),
                            # holdout - all splits
                            brnn_holdout_predictions = map(brnn_holdout_predictions, standardize_perc_change),
                            # randomGLM:
                            randomGLM_test_predictions = ifelse(split < 5,
...
cryptodata_nested <- mutate(cryptodata_nested,
                            ...
                            # Bayesian Regularized Neural Network - RMSE - Test
                            brnn_rmse_test = unlist(ifelse(split < 5,
                                                         map2(brnn_test_predictions, actuals_test, evaluate_preds_rmse),
                                                         NA)),
                            # And holdout:
                            brnn_rmse_holdout = unlist(map2(brnn_holdout_predictions, actuals_holdout, evaluate_preds_rmse)),
                            # Bayesian Regularized Neural Network - R^2 - Test
                            brnn_rsq_test = unlist(ifelse(split < 5,
                                                         map2(brnn_test_predictions, actuals_test, evaluate_preds_rsq),
                                                         NA)),
                            # And holdout:
                            brnn_rsq_holdout = unlist(map2(brnn_holdout_predictions, actuals_holdout, evaluate_preds_rsq)),
                            # randomGLM - RMSE - Test
...
<!-- rmse_scores <- dplyr::select(cryptodata_nested, lm_rmse_test, xgb_rmse_test, brnn_rmse_test, nnet_rmse_test) -->
...
<!-- rsq_scores <- dplyr::select(cryptodata_nested, lm_rsq, xgb_rsq, brnn_rsq, nnet_rsq) -->
...
rmse_test <- mutate(cryptodata_metrics,
                      ...
                      brnn = mean(brnn_rmse_test, na.rm = T),
...
rmse_holdout <- mutate(cryptodata_metrics,
                      ...
                      brnn = mean(brnn_rmse_holdout, na.rm = T),
...
rsq_test <- mutate(cryptodata_metrics,
                      ...
                      brnn = mean(brnn_rsq_test, na.rm = T),
...
rsq_holdout <- mutate(cryptodata_metrics,
                      ...
                      brnn = mean(brnn_rsq_holdout, na.rm = T),
