...
```{r upload_results_pin, error=TRUE, include=FALSE}
# register board
...
# Now create new dataset with the predictions where target price was still unknown
unknown_prices <- filter(select(unnest(filter(cryptodata_nested, split == 5), cols = c(holdout_data, lm_holdout_predictions, xgb_holdout_predictions, xgbTree_holdout_predictions, nnet_holdout_predictions)), date_time_utc:target_price_24h, lm_holdout_predictions, xgb_holdout_predictions, xgbTree_holdout_predictions, nnet_holdout_predictions), is.na(target_price_24h))
# Pin the new data
pin(plot_scores, board='github', name='crypto_tutorial_predictions_unknown_target_latest')
```
