...
lm_holdout_preds <- select(mutate(unnest(mutate(cryptodata_nested,
                                  ...
                                  model = 'lm'), holdout_predictions:lm_rmse_holdout)
# xgb:
xgb_holdout_preds <- select(mutate(unnest(mutate(cryptodata_nested,
                                  ...
                                  model = 'xgb'), holdout_predictions:xgb_rmse_holdout)
# nnet:
nnet_holdout_preds <- select(mutate(unnest(mutate(cryptodata_nested,
                                  ...
                                  model = 'nnet'), holdout_predictions:nnet_rmse_holdout)
# random forest:
rf_holdout_preds <- select(mutate(unnest(mutate(cryptodata_nested,
                                  ...
                                  model = 'rf'), holdout_predictions:rf_rmse_holdout)
# PCR:
pcr_holdout_preds <- select(mutate(unnest(mutate(cryptodata_nested,
                                  ...
                                  model = 'pcr'), holdout_predictions:pcr_rmse_holdout)

# Now union all data together:
full_holdout_preds <- gtools::smartbind(lm_holdout_preds, xgb_holdout_preds, nnet_holdout_preds, rf_holdout_preds, pcr_holdout_preds)
```

<!-- And finally pin it so it can be read it by RStudio Server process -->

```{r upload_full_predictions_pin, include=F}
# register board
board_register("github", repo = "predictcrypto/pins", token=pins_key)
# Add current date time
full_holdout_preds$last_refreshed <- Sys.time()
# pin data
pin(full_holdout_preds, board='github', name='crypto_tutorial_holdout_predictions')
```
