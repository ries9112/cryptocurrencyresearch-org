...
<!-- 11/09 - MAKE SURE IT'S OK TO HAVE COMMENTED OUT THIS STEP -->
...
The number of rows for the `cryptodata` dataset before the filtering step was `r rowcount1` and is now `r rowcount2`. This step removed `r cryptocount1-cryptocount2` cryptocurrencies from the analysis that did not have enough observations associated with them.
...
The simplest method of doing this is to take a larger majority of the data and assign it to be the [**train dataset**]{style="color: purple;"}, which the predictive models use to learn the statistical patterns in the data that can then be used to make predictions about the future. Then using the rest of the data which has not yet been seen, which is the [**test dataset**]{style="color: purple;"}, we can assess if the statistical models work on the new data in the way that we would expect based on the results obtained on the train dataset. If the results are consistent between the two, this is a good sign. If the results on the test data are much worse than those obtained on the train data, this could be a sign of [[**overfitting**]{style="color: purple;"}](https://en.wikipedia.org/wiki/Overfitting) which means the model created overspecialized itself on the training data, and it is not very good at predicting new data because it learned the exact patterns of the training data instead of understanding the actual relationships between the different variables.

(explain step below)

We will not walk through the steps below in detail outside of the comments left throughout the code...ADD
...
(should explain why in our case we made our own method because needed to pick size of each split based on the individual cryptocurrency and we were not finding anything that worked well for us. Many different ways of doing this, but most important thing is to understand the goal of why we are doing this and not forgetting the importance of actually confirming the trained models work well on data outside of their own, and helping them avoid being overly trained to data from a given time period)
...
The chart above looks strange, we can view the results for the **BTC** cryptocurrency by running the same code as above, but instead of visualizing the dataset as a whole `cryptodata`, we will limit the data using `filter(cryptodata, symbol == "BTC")`. This time we are not saving these results as an R object.
```{r}
...
The code above might seem difficult, but there is a standardized mathod to using these tools that is relatively easy to learn as we will see in the [next section](#visualization).
...
minimum_sd <- filter(distinct(mutate(group_by(ungroup(find_zero_var), symbol),
                                     min_sd = min(min_sd, na.rm=T)), min_sd),min_sd < 0.00001)$symbol
# Show result
minimum_sd
```
