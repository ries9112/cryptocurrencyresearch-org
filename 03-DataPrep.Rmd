...
We can confirm that the [**trade_usd_1**]{style="color: blue;"} field 
```{r}
...
Let's also remove any rows that have Null values for the new [**price_usd**]{style="color: blue;"} field [like we did for the [**date_time_utc**]{style="color: blue;"} field in a previous step](#remove-nulls).
...
cryptodata <- filter(cryptodata, n() >= 700)
```
