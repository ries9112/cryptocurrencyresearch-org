---
title: "cryptocurrencyresearch </br> </br> </br> High-Level Tutorial"
output: 
...
A cryptocurrency is usually defined as a digital currency based on [blockchain technology]{style="color: purple;"} which has properties that make it difficult to be exploited. The most famous and original cryptocurrency is [Bitcoin]{style="color: purple;"}, you can [view the original whitepaper for it here](https://bitcoin.org/bitcoin.pdf).

Cryptocurrencies can be traded on [cryptocurrency exchanges](https://coinmarketcap.com/rankings/exchanges/), and unlike the stock market they are available for trading 24/7. In this tutorial we will use data for the second most well known cryptocurrency, which is called **Ethereum** and goes by the trading ticker symbol of **ETH**. 


## Recommended Usage

- We recommend viewing the presentation in full-screen by pressing the **`f`** key on your keyboard.

- Each slide has a unique link. If you click on a hyperlink and lose track of which slide you were on, go back to the previous page using your web browser: 
![](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/back_button.PNG)\
...
![](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/rstudio_interface.PNG){width="800"}
...
![](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/rstudio_interface_annotated.PNG)
...
![](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/rstudio_assign_variable_2.PNG){width="705"}
...
![](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/rstudio_help.PNG){width="529"}
...
**Alternatively, you can** <a href="https://mybinder.org/v2/gh/ries9112/high-level-reprex-jupyter/c48ab7fdf606eee68b30b27c2ed5be01ec5fc19f?filepath=high-level_jupyter.ipynb" target="_blank">**click here to run the code in the cloud without having to worry about installing anything** </a>.
...
We can install the package called [**pacman**]{style="color: #ae7b11;"} [[@R-pacman]](#pacman-citation) using [install.packages()]{style="color: green;"}:
...
<span style="font-size: 90%;">
By running the [board\_register()]{style="color: green;"} command on the URL where the data is located, we will be able to ***"ask"*** for any dataset available on the board (in the next slide ⬇️).
</span>
...
In the [full version of this tutorial we are a bit more thoughtful around EDA](https://cryptocurrencyresearch.org/explore-data.html#data-quality).

## Dplyr Package

<span style="font-size: 80%;">
There are resources and tools out there that are very effective for cleaning and manipulating data. We recommend starting with the [**dplyr**]{style="color: #ae7b11;"} package [[@R-dplyr]](#dplyr-citation) if working in R, which is what we used to clean the data. Because this is such a useful tool in R, we included a brief example using the [**mutate()**]{style="color: green;"} function to add a new column to the data that calculates the [**spread**]{style="color: blue;"} for each row, which we will define as the percent difference between the [Low]{style="color: blue;"} and the [High]{style="color: blue;"} prices.
</span>
...
[![Illustration by Allison Horst](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/dplyr_mutate.png){width="48%"}](https://dplyr.tidyverse.org/reference/mutate.html)
...
Using [**mutate()**]{style="color: green;"} we can add a new column called [**spread**]{style="color: blue;"}, which tracks the percent difference between the [Low]{style="color: blue;"} price and the [High]{style="color: blue;"} price at each hourly datapoint as an absolute value:

```{r add_spread}
cryptodata <- mutate(cryptodata, spread = abs(((High - Low)/Low)*100))
```

<!-- Here css code to create scrollable output code boxes instead of printing really long outputs -->
```{css, echo=FALSE}
.scroll-150 {
  max-height: 150px;
  overflow-y: auto;
...
<span style="font-size: 56%;">
Later we will introduce a more powerful way of doing this called [**cross validation**]{style="color: #purple;"} which repeats the process over random samples of the data. Because our data is captured over a period of time, we will consider *when* the data was collected in the way we split, and do [**time aware cross validation**]{style="color: #purple;"}.
</span>

<!-- Later on we will discuss a slightly better way called ***cross validation***. -->

## Train/Test Split - Continued

<small>
Let's split the data into a [**train**]{style="color: purple;"} and [**test**]{style="color: purple;"} dataset. We will build the models on the earlier 80% of the data, and assess how well the models perform on the last 20% of the data. For now this is a simplified version of the process, which will ultimately split the data into more subsets and consider the specific date on which the data was collected in those splits.
</small>
...
- [nrow()]{style="color: green;"} returns the total number of rows in the data.

- [as.integer()]{style="color: green;"} removes decimals from `nrow(cryptodata)*.8)`.

- [head()]{style="color: green;"} returns the specified number of elements based on the other two operations.

- [tail()]{style="color: green;"} does the opposite of [head()]{style="color: green;"} and gets the last elements.
...
[![Illustration by Allison Horst](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/ggplot2_masterpiece.png){width="44%"}](https://ggplot2.tidyverse.org/)
...
For example, we could use the [**ggthemes**]{style="color: #ae7b11;"} package, [[@R-ggthemes]](#ggthemes-citation) which was [installed and loaded previously](#use-pacman), to change the look of the chart:
...
We cannot cover all these extensions in detail here, but as one last example here we use the [**transition_states()**]{style="color: green;"} function from the [**gganimate**]{style="color: #ae7b11;"} package [[@R-gganimate]](#gganimate-citation) to create an animation that iterates through each date and adjusts the axis relative to the current data:
...
<small> We wrapped [**animated\_chart**]{style="color: blue;"} in the function [**animate()**]{style="color: green;"} to slow down and resize the GIF shown. We will stop here, but we encourage you to keep exploring the possibilities of [**ggplot2**]{style="color: #ae7b11;"} in the [full version of the tutorial](https://cryptocurrencyresearch.org/visualization.html). Move on to the next slide `r emo::ji('right arrow')` to start making predictive models! </small>
...
```{r show_caret_models_list, echo=F, out.width='90%', out.height='400px'}
...
![](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/AvailableModels_InDepth.png)
...
<small>
You may have noticed that both outputs say ***Resampling: Bootstrapped (25 reps)***. This is because of a default value on an additional option of <span style="color: green;">**train()**</span>, go to the next slide below to learn more about additional options/functionality.
</small>
...
[![](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/caret_cross_validate.png){width=62%}](https://topepo.github.io/caret/data-splitting.html#data-splitting-for-time-series)
...
In the [full-version of the tutorial](https://cryptocurrencyresearch.org/evaluate-model-performance.html#eval-rmse), we will compare the scores for the RMSE and $R^2$ metrics across many cryptocurrencies and models over time.
...
<span style="font-size: 80%;">
We can create a <span style="color: #ae7b11;">**tsibble**</span> object using the <span style="color: green;">**as_tsibble()**</span> function, and using the <span style="color: blue;">**DateTime**</span> field as the <span style="color: blue;">**index**</span> of the tsibble:</span>
```{r make_tsibble}
...
If we print the new object that we created in the previous slide <span style="color: blue;">**cryptodata_ts**</span>, the top of the output will now show the text **"A tsibble: 5,111 x 18 [1h] \<UTC\>"** which specifies the dimensions (rows X columns), 1 hour intervals between rows, and the UTC timezone:
...
We can find the specific gaps (if there are any) using the <span style="color: green;">**scan_gaps()**</span> function:
```{r scan_gaps}
...
<span style="font-size: 80%;">
[As was already discussed, this is an example to illustrate the idea behind creating a train/test split to assess the performance of the models on data that the model has not seen before (trained on "train" data, and assessed on "test" data), but in practice a more complicated implementation using cross validation is much more effective.](#traintest-split)
</span>

# Timeseries Predictive Modeling

Next, we will use the <span style="color: #ae7b11;">**fable** </span> package [[@R-fable]](#fable-citation) to create timeseries models. Why is it called fable?

1. It makes <span style="color: #ae7b11;">**f**</span>orecasting t<span style="color: #ae7b11;">**able**</span>s

2. "A fable is like a forecast: it's never true, but it tells you something important about reality" - ***Rob J Hyndman*** [New York Open Statistical Programming Meetup](https://youtu.be/yx6OQ-8HofU?t=2484)
...
```{r multiple_ts_models, class.output="scroll-150"}
set.seed(123)
...
![non-seasonal components](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/arima_pdq.png)

## ARIMA - Fable Documentation

![seasonal components](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/arima_pdq_caps.png)
...
The ARIMA model has terms as described earlier for **(p, d, q)(P, D, Q)**, shown above as <span style="color: blue;">**(0,1,4)(0,0,1)**</span>. This summarizes the ARIMA model go to the next slide to get a better understanding of what this tells us.

## ARIMA Terms - Continued

![](C:/Users/ries9/Documents/GitHub/cryptocurrencyresearch-org/images/arima_labelled.PNG){width='72%'}

<!-- As labeled in red above: -->

1. The <span style="color: green;">**ARIMA()**</span> function we used automatically picked a value for <span style="color: blue;">**p**</span> of **0**, meaning there are no non-seasonal auto-regressive terms [(as seen two slides back)](#arima-terms).

2. The <span style="color: blue;">**d**</span> term was set to **1**, meaning the data was differenced once to try and achieve stationarity [(a note on that in two slides)](#a-note-on-stationarity).
...
```{r forecasts_results, echo=F, class.output="scroll-200"}
ts_forecasts
```
...
ggplotly(ggplot(ts_forecasts, aes(x=DateTime, y=.mean, color=.model)) +
           geom_line() + 
           xlim(c(as.POSIXct('2020-06-25 14:00:00', format = "%Y-%m-%d %H:%M:%S"),
     as.POSIXct('2020-06-30 00:00:00', format = "%Y-%m-%d %H:%M:%S"))))
```
...
[Refer to this previous slide for more information on the error metrics.](#error-metrics)
...
<!-- Click the link above for a more practical  -->
...
## R Package - tidyverse {#tidyverse-citation}
...
## R Package - ggthemes {#ggthemes-citation}
...
## R Package - gganimate {#gganimate-citation}
...
## R Package - xgboost {#xgboost-citation}
...
## R Package - fable {#fable-citation}
...
- Wang, Earo, et al. “A New Tidy Data Structure to Support Exploration and Modeling of Temporal Data.” Journal of Computational and Graphical Statistics, vol. 29, no. 3, 2020, pp. 466–478., [doi:10.1080/10618600.2019.1695624](https://doi.org/10.1080/10618600.2019.1695624)

- https://rstudio.com/resources/rstudioconf-2019/melt-the-clock-tidy-time-series-analysis/

- https://people.duke.edu/~rnau/411arim.htm

- https://medium.com/@kfoofw/the-i-in-arima-modelling-and-random-walk-time-series-1fb29271e2c1
...
## Base R {#base-citation}

</br>

[@R-base]. R: A language and environment for statistical computing. R Foundation for Statistical Computing, Vienna, Austria. URL https://www.R-project.org/. 
...
<!-- Add a bunch of line breaks to remove automatic citations at the end since I am manually hyperlinking them (doesn't work well with presentation format otherwise) -->
</br>
</br>
</br>
</br>
...
<!-- Stop do parallel cluster -->

<!-- ```{r stop_cluster, include=F} -->
<!-- stopCluster(cluster) -->
<!-- ``` -->
