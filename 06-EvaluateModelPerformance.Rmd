...
<!-- NOTE 11/2: LOTS OF PROGRESS, BUT HOLDOUT NEEDS TO BE FIXED! Holdout right now is WRONG, need to grab the last price from train_data of split == 4!! ONCE I MAKE THAT UPDATE, EVERYTHING CORRECTLY STANDARDIZED AND READY TO CALC FOR ALL OTHER MODELS-->
...
cryptodata_nested <- mutate(cryptodata_nested,
                            lm_test_predictions = ifelse(split < 5,
                                                         ...
                                                         NA),
                            # Holdout for all splits (UPDATE THIS, NEED TRAIN PRICE OF 4th SPLIT)
                            lm_holdout_predictions = ifelse(split < 5,
                                             map2(test_data, lm_holdout_predictions, last_train_price),
                                             map2(train_data, lm_holdout_predictions, last_train_price)))
```
